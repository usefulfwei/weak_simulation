...
# txt文本文档句柄
f = open('RNN_restore_result_multi.txt', 'w')
...
with tf.Session() as sess:
    ...
    saver.restore(sess, './rnn_net/model.ckpt-11000')
    # tensorboard 写入路径 './rnnlog'
    writer = tf.summary.FileWriter('./rnnlog_multi', sess.graph)  # write to file
    ...
    for epoch in range(2,5):
        for i in range(11200):
            ...
            if i % 50 == 0:
                ...
                plt.draw()
                plt.pause(0.1)
        saver.save(sess, './rnn_net_multi/model.ckpt', global_step=epoch)

plt.savefig('RNN_restore_multi.jpg')
sio.savemat('RNN_restore_multi.mat', {'fog_accuracy': fog_info})
f.close()
